...
def process_s2_to_t6(lig, Ncol, NwinCol, hh, hv, vh, vv, S_in1, S_in2, M_in):
    ...
    for col in range(Ncol + NwinCol):
        k1r = (S_in1[hh][lig][2 * col] + S_in1[vv][lig][2 * col]) / sqrt_2
        k1i = (S_in1[hh][lig][2 * col + 1] + S_in1[vv][lig][2 * col + 1]) / sqrt_2
        # assigned to but never used
        # k2r = (S_in1[hh][lig][2 * col] - S_in1[vv][lig][2 * col]) / sqrt_2
        # k2i = (S_in1[hh][lig][2 * col + 1] - S_in1[vv][lig][2 * col + 1]) / sqrt_2
        # k3r = (S_in1[hv][lig][2 * col] + S_in1[vh][lig][2 * col]) / sqrt_2
        # k3i = (S_in1[hv][lig][2 * col + 1] + S_in1[vh][lig][2 * col + 1]) / sqrt_2

        # assigned to but never used
        # k4r = (S_in2[hh][lig][2 * col] + S_in2[vv][lig][2 * col]) / sqrt_2
        # k4i = (S_in2[hh][lig][2 * col + 1] + S_in2[vv][lig][2 * col + 1]) / sqrt_2
        # k5r = (S_in2[hh][lig][2 * col] - S_in2[vv][lig][2 * col]) / sqrt_2
        # k5i = (S_in2[hh][lig][2 * col + 1] - S_in2[vv][lig][2 * col + 1]) / sqrt_2
        # k6r = (S_in2[hv][lig][2 * col] + S_in2[vh][lig][2 * col]) / sqrt_2
        # k6i = (S_in2[hv][lig][2 * col + 1] + S_in2[vh][lig][2 * col + 1]) / sqrt_2
...
def s2_to_t3(S_in, M_in, Nlig, Ncol, NwinLig, NwinCol):
    ...
    ********************************************************************/
    ...
    for lig in range(Nlig + NwinLig):
        for col in range(Ncol + NwinCol):
            k1r = (S_in[hh][lig][2 * col] + S_in[vv][lig][2 * col]) / sqrt_2
            k1i = (S_in[hh][lig][2 * col + 1] + S_in[vv][lig][2 * col + 1]) / sqrt_2
            k2r = (S_in[hh][lig][2 * col] - S_in[vv][lig][2 * col]) / sqrt_2
            k2i = (S_in[hh][lig][2 * col + 1] - S_in[vv][lig][2 * col + 1]) / sqrt_2
            k3r = (S_in[hv][lig][2 * col] + S_in[vh][lig][2 * col]) / sqrt_2
            k3i = (S_in[hv][lig][2 * col + 1] + S_in[vh][lig][2 * col + 1]) / sqrt_2
