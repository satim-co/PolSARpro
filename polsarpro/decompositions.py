...
from polsarpro.util import _boxcar_core, _T3_to_C3_core
...
def freeman_dask(
    ...
    out = _compute_freeman_components(da_in)
    # out = _compute_freeman_components_dask(da_in)
    return da.compute(out)[0]


# prototype using only dask arrays
# TODO: cover all input cases
def freeman_full_dask(
    input_data: np.ndarray,
    input_poltype: str = "C3",
    boxcar_size: list[int, int] = [3, 3],
) -> list[np.ndarray, np.ndarray, np.ndarray]:
...
def _compute_freeman_components_dask_old(
    C3: da.Array,
