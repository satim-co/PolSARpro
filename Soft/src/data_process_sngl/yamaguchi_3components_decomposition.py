...
def main(in_dir, out_dir, pol_type, n_win_l, n_win_c, off_lig, off_col, sub_n_lig, sub_n_col, file_memerr, file_valid):
    ...
    """
    Main Function for the yamaguchi_3_components.
    Parses the input arguments, reads the input files, and processes the data using yamaguchi_3_components filtering.
    """
    print("********************Welcome in yamaguchi_3_components_decomposition********************")
    ...
    for Nb in range(nb_block):
        ...
        span_min, span_max = determination(sub_n_lig, n_polar_out, sub_n_col, m_in, valid, n_win_l, n_win_c, n_win_lm_1s2, n_win_cm_1s2)
    ...
    for Nb in range(nb_block):
        ...
        if flag_valid == 1:
            valid = util_block.read_block_matrix_float(in_valid, valid, Nb, nb_block, sub_n_lig, sub_n_col, n_win_l, n_win_c, off_lig, off_col, n_col)
        ...
        m_odd, m_dbl, m_vol = yamaguchi_3_components(n_polar_out, sub_n_lig, sub_n_col, n_win_l, n_win_c, n_win_lm_1s2, n_win_cm_1s2, m_in, valid, span_min, span_max, m_odd, m_dbl, m_vol)
...
def open_output_files(out_dir):
    ...
    """
    try:
        out_odd = open(os.path.join(out_dir, "Yamaguchi3_Odd.bin"), "wb")
    except IOError:
        print(f"Could not open output file : {os.path.join(out_dir, 'Yamaguchi3_Odd.bin')}")
        raise

    try:
        out_dbl = open(os.path.join(out_dir, "Yamaguchi3_Dbl.bin"), "wb")
    except IOError:
        print(f"Could not open output file : {os.path.join(out_dir, 'Yamaguchi3_Dbl.bin')}")
        raise

    try:
        out_vol = open(os.path.join(out_dir, "Yamaguchi3_Vol.bin"), "wb")
    except IOError:
        print(f"Could not open output file : {os.path.join(out_dir, 'Yamaguchi3_Vol.bin')}")
        raise
...
def set_valid_pixels(valid, flag_valid, sub_n_lig, sub_n_col, n_win_c, n_win_l):
    """
    Set the valid pixels for the yamaguchi_3_components filter based on the provided parameters.
    """
    if not flag_valid:
...
# %% [codecell] yamaguchi_3_components
@jit(nopython=True)
def yamaguchi_3_components(n_polar_out, sub_n_lig, sub_n_col, n_win_l, n_win_c, n_win_lm_1s2, n_win_cm_1s2, m_in, valid, span_min, span_max, m_odd, m_dbl, m_vol):
    """
    Perform yamaguchi_3_components filtering on the input data, updating the output matrix with the filtered values.
    """
    for lig in range(sub_n_lig):
        ...
        for col in range(sub_n_col):
            if valid[n_win_lm_1s2+lig][n_win_cm_1s2+col] == 1.:
                ...
                if m_odd[lig][col] < span_min: m_odd[lig][col] = span_min
                if m_dbl[lig][col] < span_min: m_dbl[lig][col] = span_min
                if m_vol[lig][col] < span_min: m_vol[lig][col] = span_min

                if m_odd[lig][col] > span_max: m_odd[lig][col] = span_max
                if m_dbl[lig][col] > span_max: m_dbl[lig][col] = span_max
                if m_vol[lig][col] > span_max: m_vol[lig][col] = span_max
            else:
                m_odd[lig][col] = 0
                m_dbl[lig][col] = 0
                m_vol[lig][col] = 0
...
def determination(sub_n_lig, n_polar_out, sub_n_col, m_in, valid, n_win_l, n_win_c, n_win_lm_1s2, n_win_cm_1s2):
    for lig in range(sub_n_lig):
...
def yamaguchi_3_components_algorithm(CC11, CC13_re, CC13_im, CC22, CC33, eps):
    FV = 3. * CC22 / 2.
    CC11 = CC11 - FV
    CC33 = CC33 - FV
