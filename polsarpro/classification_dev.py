...
def wishart_h_a_alpha(
    ...
    # Convert xarray dataset to Dask array to avoid framework mixing
    M = _reconstruct_matrix_from_ds(in_) + 1e-12 * da.eye(n, dtype="complex64")
    ...
    for iteration in range(max_iter):
        ...
        M_center = (mask[:, :, None, None, :] * M[:, :, :, :, None]).sum((0, 1)) / npts
        M_center = M_center.transpose((2, 0, 1))
        ...
        if total_pixels > 0:
            ...
            # percent_changed = ((changed_pixels / total_pixels) * 100.0).compute()
...
def _trace_product_3(M_inv: da.Array, cov: da.Array) -> da.Array:
    """
    ...
    Args:
        ...
        cov_ds: Data array containing covariance/coherency matrix elements
...
def _trace_product_4(M_inv: da.Array, cov: da.Array) -> da.Array:
    """
    ...
    Args:
        ...
        cov: Array containing covariance/coherency matrix elements
